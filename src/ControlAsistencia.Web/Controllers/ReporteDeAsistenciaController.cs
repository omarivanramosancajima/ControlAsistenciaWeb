...
public class ReporteDeAsistenciaController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> ExportarExcelFalta(DateTime? fechaDesde, DateTime? fechaHasta, string? persona, string? area, int? areaDeptId, string? estado)
    {
        var report = await _service.GetReportAsync(new AttendanceReportRequest
        {
            FechaDesde = fechaDesde,
    ...
    [HttpGet]
    public async Task<IActionResult> EmitirReporteFalta(DateTime? fechaDesde, DateTime? fechaHasta, string? persona, string? area, int? areaDeptId, string? estado)
    {
        var report = await _service.GetReportAsync(new AttendanceReportRequest
        {
            FechaDesde = fechaDesde,
        ...
        var pdf = Document.Create(container =>
        {
            foreach (var person in persons)
            {
                container.Page(page =>
                {
                    ...
                    {
                        column.Spacing(4);
                        column.Item().Text("Informe de Faltas del Personal").Bold().FontSize(12).AlignCenter();
                        column.Item().Text($"Rango: {reportFrom:yyyy-MM-dd} a {reportTo:yyyy-MM-dd}").FontSize(8).AlignCenter();
                        column.Item().Text("(Refrigerio => HV, HR ó HN: horario que descuenta 45, 60 ó 90 minutos, HS o cualquier otro: horario que no descuenta.)").FontSize(7).AlignCenter();
                        column.Item().PaddingTop(2).Text($"RUC: {report.CompanyTaxId} - {report.CompanyName}").Bold().FontSize(8);
                        //column.Item().Text($"{report.CompanyTaxId} - {report.CompanyName}").FontSize(8);
                        ...
                        */
                        ...
                        column.Item().PaddingTop(4).Text($"Total Faltas de {person.Personal} : "+person.DiasFalta+" días.").Bold().FontSize(8.5f);
                    });
